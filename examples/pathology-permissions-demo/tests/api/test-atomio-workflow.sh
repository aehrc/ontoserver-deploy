...
# Set ATOMIO_URL to enable (default: https://localhost:9083).
...
status=$(curl -sk -o /dev/null -w "%{http_code}" "${ATOMIO_URL}/actuator/health" 2>/dev/null)
assert_http "Atomio health endpoint is accessible" "200" "$status"

# ---- List feeds ----

feeds=$(curl -sfk "${ATOMIO_URL}/feed" 2>/dev/null)
...
aliases=$(curl -sfk "${ATOMIO_URL}/alias" 2>/dev/null)
...
status=$(curl -sk -o /dev/null -w "%{http_code}" "${ATOMIO_URL}/alias/uat/syndication.xml" 2>/dev/null)
assert_http "UAT alias syndication feed accessible" "200" "$status"

status=$(curl -sk -o /dev/null -w "%{http_code}" "${ATOMIO_URL}/alias/production/syndication.xml" 2>/dev/null)
assert_http "Production alias syndication feed accessible" "200" "$status"

# ---- Create a test feed ----

TEST_FEED="test-feed-$$"
status=$(curl -sk -o /dev/null -w "%{http_code}" -X POST "${ATOMIO_URL}/feed" \
    -H "Content-Type: application/json" \
    -d "{\"name\": \"${TEST_FEED}\", \"title\": \"Test Feed\"}" 2>/dev/null)
assert_http "Can create a new feed" "201" "$status"

# Verify it exists
feed_exists=$(curl -sfk "${ATOMIO_URL}/feed" 2>/dev/null | jq "any(.[]; .name == \"${TEST_FEED}\")" 2>/dev/null || echo "false")
assert_eq "Newly created feed exists" "true" "$feed_exists"

# ---- Create a test alias ----

TEST_ALIAS="test-alias-$$"
status=$(curl -sk -o /dev/null -w "%{http_code}" -X POST "${ATOMIO_URL}/alias" \
    -H "Content-Type: application/json" \
...
status=$(curl -sk -o /dev/null -w "%{http_code}" -X PUT "${ATOMIO_URL}/alias/${TEST_ALIAS}" \
    -H "Content-Type: application/json" \
...
target=$(curl -sfk "${ATOMIO_URL}/alias" 2>/dev/null | jq -r ".[] | select(.aliasName == \"${TEST_ALIAS}\") | .feedName" 2>/dev/null)
...
curl -sfk -o /dev/null -X DELETE "${ATOMIO_URL}/alias/${TEST_ALIAS}" 2>/dev/null || true
curl -sfk -o /dev/null -X DELETE "${ATOMIO_URL}/feed/${TEST_FEED}" 2>/dev/null || true
curl -sfk -o /dev/null -X DELETE "${ATOMIO_URL}/feed/${CLONE_FEED}" 2>/dev/null || true

# ---- Gamma content feed exists ----

has_gamma=$(curl -sfk "${ATOMIO_URL}/feed" 2>/dev/null | jq 'any(.[]; .name == "gamma-content")' 2>/dev/null || echo "false")
assert_eq "Atomio has 'gamma-content' feed" "true" "$has_gamma"

# ---- Swagger UI is accessible ----

status=$(curl -sk -o /dev/null -w "%{http_code}" "${ATOMIO_URL}/swagger-ui/index.html" 2>/dev/null)
